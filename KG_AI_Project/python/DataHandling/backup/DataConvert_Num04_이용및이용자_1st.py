...
columns_2014_2019 = [
    ...
    '대출책수','재학생수(전년도)','재학생 1인당 대출책수',
    '방문자수','재학생수(전년도)','재학생 1인당 도서관방문자수'
]
...
for sheet in sheet_names:
    ...
    # 열 슬라이싱 (연도별 열 수에 따라 조정)
    if year <= 2019:
        subset = df.iloc[7:, 0:len(columns_2014_2019)]
        subset.insert(0, '연도', year)
        subset.columns = ['연도'] + columns_2014_2019
    elif year == 2020:
        subset = df.iloc[7:, 0:len(columns_2020)]
        subset.insert(0, '연도', year)
        subset.columns = ['연도'] + columns_2020
    else:
        subset = df.iloc[7:, 0:len(columns_2021_2024)]
        subset.insert(0, '연도', year)
        subset.columns = ['연도'] + columns_2021_2024

    # 필요시 float 변환 컬럼 지정
    # subset['재학생 1인당 도서관방문자수'] = pd.to_numeric(...)

    # CSV 저장
    subset.to_csv(f'Num04_이용및이용자_{year}.csv', index=False, encoding='utf-8-sig')
